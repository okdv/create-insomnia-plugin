...
export const packageInit = (
    {packageManager, devDependencies, logLevel, templatePath} : 
    {packageManager: PackageManager, devDependencies: string, logLevel?: string, templatePath?:string}) => {
        const argsArr = []
        if (packageManager === 'yarn') {
            ...
            argsArr.push(["install" , "--save-exact", "--save-dev", "--no-audit", devDependencies])
            if (templatePath) {
